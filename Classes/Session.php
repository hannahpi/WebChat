...
class Session {
    ...
    private function interpretItem($dbRow) {
        $dbSession = array(
            ...
            "lastMessageId" => $dbRow["LastMessageId"],
    ...
    public function createNew($userID, $ipAddr, $lastAction="login", $lastActionTime=NULL, $loginTime=NULL) {
        ...
        $query = " INSERT INTO Session (UserID, IP_Address, Fingerprint, LastMessageId, LastActionTime, LoginTime) "
                ." VALUES (:userID, :ipAddr, :fingerprint, :lastMessageId, :lastActionTime, :loginTime ) ;";
        ...
        $stmt->bindValue(":lastMessageId", $this->lastMessageId, PDO::PARAM_INT);
    ...
    public function setAction($action) {
        $this->lastMessageId= $action;
        $this->lastActionTime = time();
        $this->dirty = true;
    }

    public function getBySession($sessionID, $json=false) {
        $query = "SELECT SessionID, UserID, IP_Address, Fingerprint, LastMessageId, LastActionTime, LoginTime "
               . " FROM Session "
        ...
        if ($json) {
            ...
            $this->lastMessageId= $row["LastMessageId"];
    ...
    public function updateDB() {
        if (isset($this->sessionID) && $this->dirty) {
            $query = " Update `Session` set IP_Address = :ipAddr, Fingerprint = :fingerprint, "
                   . " LastMessageId= :lastMessageId, LastActionTime = :lastActionTime, "
            ...
            $stmt->bindValue(":ipAddr", $this->ipAddr, PDO::PARAM_STR);
            $stmt->bindValue(":fingerprint", $this->fingerprint, PDO::PARAM_STR);
            $stmt->bindValue(":lastMessageId", $this->lastAction, PDO::PARAM_INT);
            $stmt->bindValue(":lastActionTime", $this->lastActionTime, PDO::PARAM_INT);
